...
export async function addPhoto(
  ...
  if ("message" in uploadedPhoto) return { message: [uploadedPhoto.message] };

  const data = {
    caption: payload.get("caption") as string,
    name: uploadedPhoto.image,
    date: payload.get("date") as unknown as Date,
    category: category,
  };
  const res = await postData("/photos", data);

  if ("message" in res) return { message: res.message };
